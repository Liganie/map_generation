...
function visualizeFeatures(svg, render, params) {
    ...
    if(params.features.cities.doCities) {
        render.cities.positions = []; // the cities position will be modified to ensure the markes stay inland
        for(var c=0; c<render.cities.length; c++) {
            var city_type = c >= params.engine.population.numberTerritories;
            var base = [render.h.mesh.vxs[render.cities[c]][0], render.h.mesh.vxs[render.cities[c]][1] ];
            if(render.h.mesh.tris[render.cities[c]].length=3) {
                base = [ (render.h.mesh.tris[render.cities[c]][0][0] + render.h.mesh.tris[render.cities[c]][1][0] + render.h.mesh.tris[render.cities[c]][2][0] )/3,
                         (render.h.mesh.tris[render.cities[c]][0][1] + render.h.mesh.tris[render.cities[c]][1][1] + render.h.mesh.tris[render.cities[c]][2][1] )/3   ];
            }
            ...
            render.cities.positions[c] = [base[0]+width/2, base[1]-0.015]; // The label is relative to the center of the first floor of the city marker
...
function visualizeCities(svg, render) {
    ...
    for(var i=0; i<render.cities.length; i++) {
        if(render.h.mesh.tris[render.cities[i]].length<3) {
            points[i] = [render.h.mesh.vxs[render.cities[i]][0], render.h.mesh.vxs[render.cities[i]][1]];
        }
        else {
            points[i] = [ (render.h.mesh.tris[render.cities[i]][0][0] + render.h.mesh.tris[render.cities[i]][1][0] + render.h.mesh.tris[render.cities[i]][2][0] )/3,
                          (render.h.mesh.tris[render.cities[i]][0][1] + render.h.mesh.tris[render.cities[i]][1][1] + render.h.mesh.tris[render.cities[i]][2][1] )/3   ];
        }
        radius[i] = i >= render.params.engine.population.numberTerritories ? 4 : 10;
    }
    render.cities.positions = points;
